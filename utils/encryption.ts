...
export const deriveKey = (userInfo: UserInfo): string => {
  const salt: string = CryptoJS.SHA256(userInfo.email).toString(); // Hash email to get a salt
  const iterations: number = 100000; // Adjust iterations for better security
